# --- Previous implementation, kept for reference/rollback -------------------
...
fast_median <- function(x) {
  n <- length(x)
  s <- sort.int(x, method = "quick")
  if (n %% 2 == 1) s[(n + 1) / 2] else (s[n / 2] + s[n / 2 + 1]) / 2
}

fast_mad <- function(x, center = fast_median(x)) {
  fast_median(abs(x - center)) * 1.4826
}
...
flagger <- function(x, j, k = 8, warmup = 12, stuck_threshold = 8, consec_threshold = 24, physical_min = 10, physical_max = 5000) {
  ...
  for (i in seq_along(x)) {
    ...
    # check for repeat values
    if(!is.na(prev_raw) && round(x[i], 1) == prev_round) {
      repeat_count <- repeat_count + 1
    } else {
      repeat_count <- 0
    }
    ...
    # stats flag (inlined hampel check, center shared with mad's default)
    center <- fast_median(good)
    spread <- fast_mad(good, center)
    threshold <- max(center * .1, spread * k)
    suspect <- x[i] > center + threshold || x[i] < center - threshold
    jump <- abs(prev_raw - x[i]) > j
    if(suspect & jump ) {
      flag[i] <- TRUE
      annotation[i] <- "hampel & jump"
